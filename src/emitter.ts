...
export class Emitter<T> extends Disposable {
    ...
    on<K extends keyof T>(name: K, callback: EmitterCallbackFn<T[K]>, scope: object = this): DisposeFn {
        this._addCallback(name, callback, scope, false);

        return () => this.off(name, callback, scope);
    }

    once<K extends keyof T>(name: K, callback: EmitterCallbackFn<T[K]>, scope: object = this): DisposeFn {
        this._addCallback(name, callback, scope, true);

        return () => this.off(name, callback, scope);
    }

    off<K extends keyof T>(name: K, callback: EmitterCallbackFn<T[K]>, scope: object = this): void {
    ...
    private _addCallback<K extends keyof T>(name: K, callback: EmitterCallbackFn<T[K]>, scope: object, once: boolean) {
