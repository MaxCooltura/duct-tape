...
export abstract class Disposable implements IDisposable {
  ...
  register<T extends IDisposable | IDestroyable | IRemovable | DisposeFn>(
    o: T,
  ): T {
    if (this._disposables.has(o)) {
      console.warn(
        `Cannot register ${o?.constructor?.name ?? o}. This object is already registered.`,
      );
      return o;
    }
    ...
    if (isObject(o)) {
      if (hasOwnFunction(o, 'dispose')) {
        ...
        console.warn(
          `The object ${o?.constructor?.name ?? o} has an unknown release function!`,
        );
      }
      ...
      console.warn(
        `Cannot register ${o}. This object does not have a release function!`,
      );
    }
